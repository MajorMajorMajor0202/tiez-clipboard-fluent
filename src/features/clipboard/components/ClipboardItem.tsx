...
const ClipboardItem = ({
    ...
    const [flyoutPosition, setFlyoutPosition] = useState<{ top?: number; bottom?: number; left: number; isAbove: boolean } | null>(null);
    ...
    const [snapshotOverflows, setSnapshotOverflows] = useState(false);
    const [richTextOverflows, setRichTextOverflows] = useState(false);
    const [sourceAppIcon, setSourceAppIcon] = useState<string | null>(() => peekSourceAppIcon(item.source_app_path) ?? null);
    const filePaths = useMemo(
        () => (item.content_type === "file" || item.content_type === "video") ? item.content.split('\n').filter((p) => p.trim()) : [],
    ...
    const richTextPreviewRef = useRef<HTMLDivElement | null>(null);
    ...
    const pickableTagSuggestions = useMemo(() => {
        ...
        const existing = new Set((item.tags || []).map(tag => {
            const lower = tag.toLowerCase();
            return lower === "sensitive"
        ...
        return tagSuggestions
            ...
            .slice(0, 14);
    }, [isEditingTags, item.tags, localTagInput, tagSuggestions, t]);
    ...
    const updatePosition = useCallback(() => {
        if (!isEditingTags || !tagButtonRef.current || !onTagEditCancel) return;
        
        if (!document.body.contains(tagButtonRef.current)) {
            onTagEditCancel();
            return;
        }
        ...
        const scrollContainer = tagButtonRef.current.closest(".fluent-scrollbar-host");
        if (scrollContainer) {
        ...
        const isAbove = spaceBelow < flyoutHeight;
        
        const flyoutWidth = 192;
        let left = rect.left + rect.width / 2 - flyoutWidth / 2;
        
        if (left + flyoutWidth > window.innerWidth - 12) {
            left = window.innerWidth - flyoutWidth - 12;
        }
        if (left < 12) left = 12;

        setFlyoutPosition((prev) => {
            const next = isAbove 
                ? { bottom: window.innerHeight - rect.top + 4, left, isAbove: true }
                : { top: rect.bottom + 4, left, isAbove: false };
            
            if (prev && 
                prev.left === next.left && 
                prev.isAbove === next.isAbove && 
                prev.top === next.top && 
    ...
    useEffect(() => {
        ...
        const handleOutsideClick = (e: MouseEvent) => {
            ...
            if (target.closest(".tag-edit-flyout")) return;
    ...
    const hasTagsSection = visibleTagCount > 0;
    const overlayTagsInPreview = !compactMode && visibleTagCount > 0;
    ...
    useEffect(() => {
        ...
        if ((item.content_type !== "file" && item.content_type !== "video") || item.file_preview_exists === false || !singleFilePath) {
            setFileIcon(null);
            return () => {
                cancelled = true;
            };
    ...
    useEffect(() => {
        ...
        setSnapshotOverflows(false);
        setRichTextOverflows(false);
    }, [item.id, item.html_content, richTextSnapshotPreview, compactMode]);

    // Measure whether the HTML preview content overflows its maxHeight,
    // so we only show the fade mask when truncation is actually happening.
    useLayoutEffect(() => {
        const el = richTextPreviewRef.current;
        if (!el) return;
        const raf = requestAnimationFrame(() => {
            setRichTextOverflows(el.scrollHeight > el.clientHeight + 1);
        });
        return () => cancelAnimationFrame(raf);
    }, [item.html_content, richTextCleanHtml, richTextSnapshotDisplayMaxHeight]);
    ...
    const renderTagsContainer = (overlay = false) => (
        ...
        >
            {item.tags?.map((tag) => {
                const displayTag = tag.toLowerCase() === "sensitive"
                    ? t("tag_builtin_sensitive")
                    : (tag.toLowerCase() === "password" || tag === "密码" || tag === "密碼")
                    ? t("tag_builtin_password")
                    : tag;
                ...
                if (overlay) {
                    return (
                        <FluentTooltip key={tag} text={displayTag}>
                            <button
                                className="tag-dot"
                                onClick={(e) => {
                                    e.stopPropagation();
                            ...
                            >
                                <X size={8} className="tag-dot-delete-icon" />
                            </button>
                        </FluentTooltip>
                    );
                }
    ...
    return (
        <motion.div
            ...
            className={`history-item ${isSelected ? "selected" : ""} ${isEditingTags ? "tag-editing" : ""} ${compactMode ? "compact" : ""} ${item.is_pinned ? "pinned" : ""} ${className || ''}`}
            onMouseDown={(e) => {
                const target = e.target as HTMLElement;
                if (e.button !== 0) return;
        ...
        >
                ...
                <div className="item-meta-right">
                    <div className="item-actions">
                        {item.tags?.some(tag => {
                            ...
                            <FluentTooltip text={isRevealed ? t('hide') : t('reveal')}>
                                <button
                                    className={`btn-icon-ghost ${isRevealed ? "active" : ""}`}
                        ...
                        <FluentTooltip text={item.is_pinned ? t('unpin') : t('pin')}>
                            <button
                                className={`btn-icon-ghost ${item.is_pinned ? "active" : ""}`}
                                onClick={onTogglePin}
                            >
                                {item.is_pinned ? <PinOff size={12} /> : <Pin size={12} />}
                            </button>
                        </FluentTooltip>
                        <FluentTooltip text="Tags">
                            <button
                                ref={tagButtonRef}
                                className={`btn-icon-ghost ${isEditingTags ? "tag-editing-active" : (item.tags && item.tags.length > 0 ? "active" : "")}`}
                        ...
                        {(item.content_type === 'text' || item.content_type === 'rich_text') && aiEnabled && (
                            <FluentTooltip text={t('ai_assistant')}>
                                <button
                                    className={`btn-icon-ghost ai-btn ${isAIProcessing || showAIOptions ? 'active' : ''}`}
                                    onClick={(e) => {
                                        e.stopPropagation();
                                        if (!isAIProcessing) {
                        ...
                        <FluentTooltip text={t('delete')}>
                            <button className="btn-icon-ghost" onClick={onDelete}>
            ...
            <div className={`content-preview-shell${overlayTagsInPreview ? ' has-overlay-tags' : ''}`}>
                {item.content_type === "image" ? (
                        ...
                        <div className="video-thumbnail-wrapper">
                            {videoLoadError ? (
                                <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg-secondary)', color: 'var(--text-secondary)' }}>
                                    {fileIcon ? (
                                        <img src={fileIcon} alt="" style={{ width: '28px', height: '28px', objectFit: 'contain' }} />
                                    ) : (
                                ...
                                <video
                                    ...
                                    onLoadedData={(e) => seekVideoPreviewFrame(e.currentTarget)}
                    ...
                    richTextPreviewSrc ? (
                        <img
                            ...
                            onLoad={() => {
                                ...
                                // Only show the fade mask when the image
                                // actually exceeds the preview max height.
                                if (richSnapshotImgRef.current) {
                                    setSnapshotOverflows(
                                        richSnapshotImgRef.current.naturalHeight >
                                        richTextSnapshotDisplayMaxHeight
                                    );
                                }
                            }}
                            onError={() => {
                    ...
                    <div className={`content-preview ${isSensitiveHidden ? 'sensitive-blur' : ''}`}>
                        {isSensitiveHidden ? (
            ...
            {isEditingTags && flyoutPosition && createPortal(
                <div
                    className="tag-edit-flyout"
                    style={{
                        position: 'fixed',
                        left: `${flyoutPosition.left}px`,
                        ...(flyoutPosition.top !== undefined ? { top: `${flyoutPosition.top}px` } : { bottom: `${flyoutPosition.bottom}px` }),
                        zIndex: 99999,
                    }}
                    onMouseDown={(e) => e.stopPropagation()}
                    onClick={(e) => e.stopPropagation()}
                >
                    <div className="tag-flyout-search-wrapper">
                        <Search size={12} className="tag-flyout-search-icon" />
                        <input
                            ref={tagInputRef}
                            type="text"
                            className="tag-flyout-search-input"
                            placeholder={t('enter_tag_name') || 'Enter tag name...'}
                            value={localTagInput}
                            autoFocus
                            onCompositionStart={() => {
                    ...
                    {pickableTagSuggestions.length > 0 && onTagPick && (
                        <div
                            ...
                            className="tag-flyout-suggestions hide-scrollbar"
                            role="listbox"
                            onWheel={(e) => e.stopPropagation()}
                        >
                            {pickableTagSuggestions.map((sTag, sIdx) => {
                                const dbKey = sTag === t("tag_builtin_sensitive")
                                    ? "sensitive"
                                    : sTag === t("tag_builtin_password")
                                    ? "password"
                                    : sTag;
                                const bg = tagColors?.[dbKey] || getTagColor(dbKey, theme);
                                return (
                                    <button
                                        key={sTag}
                                        type="button"
                                        ...
                                        className={`tag-flyout-suggest-item${tagSuggestIndex === sIdx ? ' active' : ''}`}
                                    ...
                                    >
                                        <span
                                            className="tag-flyout-color-dot"
                                            style={{ backgroundColor: bg }}
                                        />
                                        <span className="tag-flyout-text">
                                            {sTag}
                                        </span>
                                    </button>
                                );
