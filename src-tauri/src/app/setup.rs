...
use crate::app::window_manager::{release_win_keys, restore_last_focus, reapply_vibrancy, toggle_window};
use crate::app_state::{
...
use windows::Win32::UI::WindowsAndMessaging::{
    ...
    SetWindowPos, SWP_NOMOVE, SWP_NOSIZE, SWP_NOZORDER, SWP_NOACTIVATE, SWP_FRAMECHANGED,
};
...
pub fn init(app: &mut App) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 9.1 System Theme Watcher (detects PowerToys Light Switch etc.)
    #[cfg(target_os = "windows")]
    watch_system_theme(app_handle.clone());
        ...
        .on_tray_icon_event(|tray, event| {
            ...
            } = event
            {
                if let Some(window) = tray.app_handle().get_webview_window("main") {
                    let _ = window.show();
                    let _ = window.emit("window-shown", ());
                    reapply_vibrancy(tray.app_handle());
...
/// Background thread that watches the Windows registry for system theme changes
/// (e.g. from PowerToys Light Switch) and emits a `system-theme-changed` event
/// so the frontend can react even when Tauri's built-in `onThemeChanged` and
/// WebView2's `matchMedia` change events don't fire.
#[cfg(target_os = "windows")]
fn watch_system_theme(app_handle: AppHandle) {
    use winreg::enums::*;
    use winreg::RegKey;
    ...
    std::thread::spawn(move || {
        ...
        loop {
            ...
            let hkcu = match RegKey::predef(HKEY_CURRENT_USER).open_subkey_with_flags(
            ...
            let current: u32 = match hkcu.get_value("AppsUseLightTheme") {
                Ok(v) => v,
                Err(_) => continue,
            };

            if last_value.is_some() && last_value.unwrap() != current {
                let theme = if current == 0 { "dark" } else { "light" };
...
pub fn handle_global_shortcut(app: &AppHandle, shortcut: &tauri_plugin_global_shortcut::Shortcut) {
    ...
    if let Ok(main_s) = {
        ...
        val.replace("Win", "Super").replace("Command", "Super").replace("Cmd", "Super").parse::<Shortcut>()
    } {
        if shortcut == &main_s {
            toggle_window(app);
            return;
...
pub fn handle_window_event(window: &tauri::Window, event: &tauri::WindowEvent) {
    match event {
        ...
        tauri::WindowEvent::Resized(size) => {
            ...
            // Re-apply Mica/Acrylic when the flag is set (set by reapply_vibrancy after
            // ShowWindow). This fires after DWM has fully composed the window, which is
            // the most reliable moment to apply SYSTEMBACKDROP_TYPE.
            if size.width > 0 && size.height > 0
                && VIBRANCY_NEEDS_REAPPLY.swap(false, Ordering::Relaxed)
            {
                reapply_vibrancy(window.app_handle());
            }
